        ...
        <div class="imglogo">
            ...
            <div class="saludo" id="userMenu">
                ...
                <div class="dropdown-menu" id="dropdown">
                    <a href="../../auth/logout.php"><i class="bi bi-box-arrow-left"></i> Cerrar sesión</a>
                </div>
            </div>
        </div>

        <div class="main" id="menuPrincipal">
            <div class="orgmain"><a href="../Usuario/FormularioUser.php" class="menu-card"><div class="icon"><i class="bi bi-person-gear"></i></div><h3>Usuarios</h3><p>Gestión de usuarios</p></a></div>
            <div class="orgmain"><a href="../Categoria/FormularioCat.php" class="menu-card"><div class="icon"><i class="bi bi-bookmark-plus"></i></div><h3>Categorías</h3><p>Administrar categorías</p></a></div>
            <div class="orgmain"><a href="../Subcategoria/GestorSubcategoria.php" class="menu-card"><div class="icon"><i class="bi bi-diagram-3-fill"></i></div><h3>Subcategorías</h3><p>Gestionar subcategorías de productos</p></a></div>
            ...
            <div class="orgmain"><a href="../Productos/FormularioProduc.php" class="menu-card"><div class="icon"><i class="bi bi-boxes"></i></div><h3>Productos</h3><p>Administrar productos</p></a></div>
        </div>
...
btnElimSelItems.addEventListener("click", () => {
    ...
    fetch("eliminarItemsMasivo.php", { method:"POST", body: fd })
    .then(r => r.json())
    .then(d => {
        if(d.status === "success"){
            modoSelItems = false;
...
function cargarItems(idProducto){
    ...
    fetch("getItems.php?idProducto=" + idProducto)
    .then(r => r.json())
    .then(items => {
        if(!items.length){
        ...
        listaItems.querySelectorAll(".inv-btn-del-item").forEach(b => {
            b.addEventListener("click", function(){
                ...
                fetch("deleteItem.php", { method:"POST", body: new URLSearchParams({ id: this.dataset.id }) })
                .then(r => r.json())
...
document.getElementById("btnAgregarItem").onclick = () => {
    ...
    fetch("getProductoInfoInv.php?idProducto=" + productoActual.id)
    .then(r => r.json())
    .then(info => {
        masivoInfo = info;
...
document.getElementById("btnConfirmarMasivo").addEventListener("click", function(){
    ...
    fetch("agregarItemsMasivo.php", { method:"POST", body: fd })
    .then(r => r.json())
    .then(d => {
        if(d.status === "success"){
...
formItem.addEventListener("submit", function(e){
    ...
    fetch("guardarItem.php", { method:"POST", body: fd })
    .then(r => r.json())
    .then(d => {
        if(d.status === "success"){
