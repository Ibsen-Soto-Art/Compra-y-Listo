...
<div class="contenedor">
        ...
        <div class="primeraPart">
            ...
            <div class="panelLateral" id="panelLateral">
                ...
                <div class="panelContenido">
                    ...
                    <a class="accion" href="<?= SITE_URL ?>/api/subcategorias/plantilla" target="_blank">
            ...
            <div class="modal" id="modalImportarSub">
                <div class="modal-import">
                    ...
                    <form id="formImportarSub" enctype="multipart/form-data">
                        ...
                        <a href="<?= SITE_URL ?>/api/subcategorias/plantilla" class="modal-import-plantilla">
            ...
            <script>
                ...
                (function(){
                    ...
                    document.getElementById("formImportarSub").addEventListener("submit", function(e){
                        ...
                        resultado.innerHTML = "";
                        fetch("<?= SITE_URL ?>/api/subcategorias/importar", { method: "POST", body: new FormData(this) })
                        .then(r => r.json())
                        .then(data => {
    ...
    (function(){
        ...
        formAgregar.addEventListener("submit", function(e){
            e.preventDefault();
            fetch("<?= SITE_URL ?>/api/subcategorias/agregar", { method:"POST", body: new FormData(formAgregar) })
            .then(r => r.json())
            .then(data => {
        ...
        formEditar.addEventListener("submit", function(e){
            e.preventDefault();
            fetch("<?= SITE_URL ?>/api/subcategorias/editar", { method:"POST", body: new FormData(formEditar) })
            .then(r => r.json())
            .then(data => {
                if(data.status === "success"){
    ...
    (function(){
        ...
        document.addEventListener("click", function(e){
            ...
            fetch(`<?= SITE_URL ?>/api/subcategorias/eliminar?id=${id}`)
            .then(r => r.json())
            .then(data => {
        ...
        document.addEventListener("click", function(e){
            ...
            if(!btn) return;
            const id = btn.dataset.id;
            fetch("<?= SITE_URL ?>/api/subcategorias/toggle-estado", {
                method:"POST",
    ...
    (function(){
        ...
        confirmarBtn.addEventListener("click", function(){
            ...
            fetch("<?= SITE_URL ?>/api/subcategorias/eliminar-varias", { method:"POST", body: fd })
            .then(r => r.json())
            .then(data => {
...
<script>
let _statsSubTimer = null;
function actualizarStatsSub() {
    clearTimeout(_statsSubTimer);
    _statsSubTimer = setTimeout(() => {
        fetch('<?= SITE_URL ?>/api/subcategorias/stats')
        .then(r => r.json())
        .then(d => {
            const set = (el, val) => {
            ...
            set(document.getElementById('statSubTotal'),   d.total);
            set(document.getElementById('statSubActivas'), d.activas);
            set(document.getElementById('statSubOcultas'), d.ocultas);
        }).catch(() => {});
    }, 250);
}
</script>
